document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule) {
      const label = field.closest('[class*="field"], [class*="group"], label, div')
        ?.querySelector('label')?.textContent?.trim() || field.getAttribute('placeholder') || 'This field';
      ...
      return messages[rule] || 'Please enter a valid value.';
    }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (isValid) {
        ...
        const successMessage = contactForm.querySelector('.form-success') || (function () {
  ...
  function initScrollAnimations() {
    ...
    const candidates = document.querySelectorAll(ANIMATE_SELECTORS.join(', '));

    candidates.forEach(function (el) {
      if (!el.closest('.contact-form') && !el.closest('nav') && !el.closest('header nav')) {
    ...
    document.querySelectorAll('.' + OBSERVE_CLASS).forEach(function (el, index) {
      const siblings = el.parentNode ? Array.from(el.parentNode.children).filter(function (child) {
        return child.classList.contains(OBSERVE_CLASS);
      }) : [];

      const siblingIndex = siblings.indexOf(el);
      if (siblingIndex > 0) {
        el.dataset.animationDelay = (siblingIndex * 0.1).toFixed(1);
      }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'c0a4a79d-274f-4b8f-ad15-f17baa7ba962',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'c0a4a79d-274f-4b8f-ad15-f17baa7ba962';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'c0a4a79d-274f-4b8f-ad15-f17baa7ba962';
